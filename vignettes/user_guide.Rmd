...
`fetch_data` can retrieve feature expression data from any assay in the object (or "experiment" in SingleCellExperiment objects and "modality" in anndata objects). To retrieve feature expression data, you can pass any number of features to the `vars` parameter.
...
### Fetching Metadata with `fetch_data`
...
### Fetching Reduction Coordinates with `fetch_data`

In a similar manner to expression data, reduction coordinates to fetch are pased to `vars` using a "key" and underscore system. In this case, you would enter the key of the reduction to use, and an integer specifying the dimension of the reduction to pull from.
