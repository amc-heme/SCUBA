...
<!-- TODO: DON'T FORGET TO ADD THE FETCH_REDUCTION STUFF -->
...
<!-- Return class demo -->
<!-- Not considered to be useful. This example is covered by unique_values. -->
<!--
...
The data access functions mentioned in the previous section can be applied downstream to create new plotting functions for single-cell data. Functions created with SCUBA generics will be flexible for object classes by default. 

In the example below, a function will be created to plot density plots for a feature. The function will accept a feature and a metadata variable and will fetch data for the entered feature or variable via SCUBA access functions. Plotting code downstream will create the density plots. 

<div class="callout callout-info">
  <h4>Note</h4>
  <p>The example below will follow principles of incremental development. Each code block will show a "snapshot" of the development process, where a progressively complex output will be created. Intermediates may be ugly and/or uninterperetable, but they will be corrected in subsequent steps.</p>
</div>
...
After fetching the data, we will make a basic density plot. In this stage, it will be for the full object and not split by a metadata variable.
...
Next, we will add a fill attribute based on the metadata variable entered. 
...
<!-- COPY THE *ABOVE* AND PLACE IN A FUNCTION -->
```{r}
object <- AML_Seurat
feature <- "rna_GAPDH"
metadata <- "condensed_cell_type"
