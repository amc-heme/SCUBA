...
<!-- TODone: DON'T FORGET TO ADD THE FETCH_REDUCTION STUFF -->
<!-- TODO: ADD fetch_cells STUFF -->
<!-- TODO
  In plotting vignette, discuss the following utility functions if applicable, otherwise link to reference docs
  Consider demonstrating each in separate plotting functions 
  (reduction_dimnames won't be used in the ridge plot example)
  default_layer
  default_reduction
  default_slot
  get_all_cells
...
At this stage, simply transfer the variables defined at the beginnng to the function as parameters, and move the subsequent code into the function.
...
The process described here can be applied to the creation of any other single-cell visualization that uses expression data, metadata, or reduction coordinates. SCUBA facilitates data access in visualization functions, allowing users to access data through a call to an access function instead of doing object-specific data wrangling. Any R visualization library can be used downstream of access functions, for example [ggiraph](https://davidgohel.github.io/ggiraph/) or [plotly](https://plotly.com/r/) for interactive plots. Any plot created using SCUBA's data access functions will work with any of the object classes supported by SCUBA. SCUBA will be expanded in the future to include functions for summary statistics, facilitating the creation of single-cell plots. 