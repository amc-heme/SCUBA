...
#' DimPlot(object = pbmc_small, split_by = 'ident')
#'
DimPlot <- function(
    object,
    dims = c(1, 2),
  ...
  plots <- lapply(
    ...
    FUN = function(group) {
      plot <- Seurat::SingleDimPlot(
        data = data[, c(dims, group, split_by, shape_by)],
        dims = dim_names,
        col.by = group,
        ...
        label = FALSE,
        # Cells to highlight
        cells_highlight = cells_highlight,
        cols_highlight = cols_highlight,
        sizes_highlight = sizes_highlight,
        na_value = na_value,
        raster = raster,
