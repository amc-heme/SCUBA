...
# SCUBA Plots on Anndata Objects

```{r}
plot_feature(
  object = object,
  features = "protein_CD36",
  order = TRUE
)
...
# Create AnnData version of Full AML Object

```{r}
library(sceasy)

full_obj <- readRDS("~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_Seurat.Rds")
...
sceasy::convertFormat(
    ...
    outFile = "Full_AML_Anndata.h5ad" #"~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_Anndata.h5ad"
    )

# Save GEX to HDF5 storage first, then convert addional modalities


# Convert ADT, SIG modalities separately
adata_protein <-
    convertFormat(
      full_obj,
      from = "seurat",
...
# Load anndata object, append modalities
library(anndata)
full_adata <- read_h5ad("~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_Anndata.h5ad")

full_adata$obsm[["protein"]] <- adata_protein$to_df()
full_adata$obsm[["sig"]] <- adata_sig$to_df()

write_h5ad(
  full_adata, 
  "~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_Anndata.h5ad"
  )
```

## Testing Plotting Functions on Full AML Anndata object
...
# Speed Test for FetchData on Anndata vs. other object types
...
time <- c(time, results)
test <- c(test, rep_len("Feature_Seurat", 100))
...
time <- c(time, results)
test <- c(test, rep_len("Feature_SCE", 100))
...
time <- c(time, results)
test <- c(test, rep_len("Feature_Anndata", 100))
...
results_df$Test <- 
  plyr::revalue(
    results_df$Test, 
    c("Feature_Seurat" = "Seurat", 
      "Feature_SCE" = "SingleCellExperiment", 
      "Feature_Anndata" = "Anndata")
    )
...
ggsave(
  "./Feature_plot_comparison.png",
  device = "png",
  bg = "#FFFFFF"
  )
```

### Speed vs. number of features
```{r}
library(reticulate)
library(anndata)
full_adata <- read_h5ad("~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_Anndata.h5ad")
...
library(HDF5Array)
sce <- loadHDF5SummarizedExperiment("~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_sce/")

sobj <- readRDS("~/1A_Jordan_Lab/Objects/Full_AML_Downsampled/Full_AML_Seurat.Rds")
```
...
for (i in 1:max_n_features){
  cat("plot_feature on SCE object, n_features =", i, "\n")
  # Select features (randomly, n = i)
  features <- rownames(sce)[sample(1:nrow(sce), i)]
  trials <- 10
  trial_name <- "SingleCellExperiment"
...
# plot_feature, Anndata object
set.seed(2023)
for (i in 1:max_n_features){
  cat("plot_feature on Anndata object, n_features =", i, "\n")
  # Select features (randomly, n = i)
  features <- full_adata$var_names[sample(1:length(full_adata$var_names), i)]
  trials <- 10
  trial_name <- "Anndata"
...
ggsave(
  filename = "./plot_feature_all_objects_1-10_features.png",
  plot,
  device = "png",
  bg = "#FFFFFF"
)
...
# Additional plot types in anndata
...
## fetch_cells method for anndata objects

This method is required if the user calls dot_plot with the "idents" parameter to select a subset of group_by metadata values to include.

```{r}
object <- full_adata
meta_var <- "best_response"
meta_levels <- c("CR", "CRi", "MLFS")

object$obs[object$obs[[meta_var]] %in% meta_levels, ] |> rownames()
```
 
## Bug in plot_feature for anndata objects

The bug ocurrs when a subset of cells is passed to plot_feature via `cells`.
...
sub <- adata[adata$obs[["best_response"]] == "CR"]

sub_cells <- sub$obs_names
