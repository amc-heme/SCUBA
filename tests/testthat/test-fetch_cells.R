...
test_that("fetch_cells returns the same results for MuData objects as for Anndata R6, SingleCellExperiment and Seurat objects.", {
  seurat_cells <-
    fetch_cells(
      AML_Seurat, 
      meta_var = "seurat_clusters",
  ...
  h5mu_cells <-
    fetch_cells(
      AML_h5mu(),  
      meta_var = "seurat_clusters",
      meta_levels = unique_values(AML_h5mu(), var="seurat_clusters")
    )
  expect_equal(
    object = h5mu_cells,
    expected = seurat_cells,
